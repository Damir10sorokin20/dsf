...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        {
            //пользаватель нажал на крестик и хочет закрыть окно?
        ...
        if (ball.getPosition().x <= 0 || ball.getPosition().x + 2 * b >= BALL_RADIUS >= WINDOW_WIDTH) {
            ball_speedx = -ball_speedx;
        } 
        if (ball.getPosition().y <= 0 || ball.getPosition().y + 2 * b >= BALL_RADIUS >= WINDOW_HEIGHT) {
